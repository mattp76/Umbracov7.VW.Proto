angular.module("umbraco")
 .factory('PMGScope', [function () {
     ...
     return service;
 }]).factory('PmgVideoVWHelper', [function () {
     // the service types should match the techOrder items.
     var servicetype = {
         "volkswagenaustralia": "youtube"
     };
    ...
    function ($scope, PmgVideoHelper, PmgVideoVWHelper, contentResource, angularHelper, editorState, $q,
        ...
        $scope.setImage = function () {
            $scope.safeApply();
            $scope.config = { "controls": false, "quality": "480p", "width": "480", "height": "360", "autoplay": false, "preload": "auto", "techOrder": [PmgVideoVWHelper.getService($scope.imageUrl)], src: $scope.imageUrl };
            $scope.model.value = $scope.imageUrl;
        }

        $scope.loaded = false;
        assetsService.load(['/App_Plugins/PMGVideoPlayer/videojs.js', '/App_Plugins/PMGVideoPlayer/Players/volkswagenaustralia.js', '/App_Plugins/PMGVideoPlayer/Players/youtube.js', '/App_Plugins/PMGVideoPlayer/Players/vimeo.js', '/App_Plugins/PMGVideoPlayer/Players/dailymotion.js']).then(function () {
            console.log('setVideo !!!!!!!!!');

            var vidid = $scope.model.config.testVideo;

            console.log('setVideo !!!!!!!!!', vidid);

            $scope.loaded = true;
            if ($scope.model.value) {
                $scope.imageUrl = $scope.model.value;
                console.log('setVideo');
                $scope.setImage();
            }
        });
